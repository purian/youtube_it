class YouTubeIt
  module Upload
    ...
    class VideoUpload
      ...
      def playlists(user,options = {"max-results" => 50, "start-index" => 1})
        playlist_url = "/feeds/api/users/%s/playlists?v=2&max-results=#{options['max-results']}&start-index=#{options['start-index']}" % (user ? user : "default")
        response     = yt_session.get(playlist_url)
